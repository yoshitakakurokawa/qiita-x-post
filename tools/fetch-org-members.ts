...
async function fetchOrgMembers(orgName: string): Promise<string[]> {
  ...
  try {
    ...
    if (!jsonStr) {
      throw new Error(
        'Could not find organization data in the page. The page structure may have changed.'
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to parse JSON data: ${parseError instanceof Error ? parseError.message : String(parseError)}`
      );
    }

    // メンバーIDを抽出
    const memberIds = data.organization.memberships.edges.map((edge) => edge.node.user.urlName);
